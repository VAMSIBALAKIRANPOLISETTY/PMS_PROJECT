...
Protected maternal warning-sign rules should run before mock AI wording. Follow-up cards should ask safe context questions, such as timing, whether symptoms are worsening, whether the user is pregnant or postpartum, and whether a care team has already been contacted. They should not ask the user to self-diagnose a complication.
...
## PMS_Test2 AI Position

`PMS_Test2` uses mock AI-style wording only. A future pregnancy module in this branch should demonstrate expected wording with `MockAiInsightService` while keeping all maternal safety decisions in Java rule logic.

Mock wording may:

- Summarize user-entered pregnancy context.
- Convert rule results into plain-language care-preparation notes.
- Suggest doctor or midwife questions.
- Add trusted source links.

Mock wording must not:

- Diagnose pregnancy complications.
- Estimate fetal condition.
- Prescribe medication or dosage.
- Replace emergency or prenatal care.
- Lower or remove rule-triggered urgent warnings.
...
- Mock AI wording cannot lower maternal urgent warnings.
