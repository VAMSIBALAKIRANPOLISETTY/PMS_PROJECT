...
- Completed result shows summary first, then detailed reasons, next steps, care tips, doctor questions, and trusted links.
...
- Staff can request AI question suggestions, but suggestions are inactive until saved and reviewed.
- Staff profile is read-only in this version.

## AI Explanation

- `PMS_Test3` uses mock AI by default.
- Real provider mode is backend-only and optional.
- Provider chain is Ollama Gemma 4 31B first, OpenAI fallback second, mock fallback last.
- `ConfiguredAiInsightService` controls the chain.
- `OllamaInsightClient` calls Ollama only when `AI_MODE=provider` and `OLLAMA_API_KEY` are configured.
- `OpenAiInsightClient` is fallback only when OpenAI credentials are configured.
...
- AI wording cannot lower rule-based urgent warnings.
- Provider output is validated and falls back safely to mock output when it is missing, invalid, timed out, or unsafe.
...
- Did you use real AI?
  In `PMS_Test3`, real AI is optional. Mock is the default. Provider mode can use Ollama Gemma 4 31B, then OpenAI, then mock fallback.

- Why keep mock fallback?
  It keeps demos and tests predictable, and it prevents provider failures from breaking care-preparation flows.
...
- What can staff do?
  Staff can review completed assessments, see analytics, manage rules, manage questions, request AI question drafts, and view staff profile details.
